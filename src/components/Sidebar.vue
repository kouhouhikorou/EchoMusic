...
<template>
  <aside
    class="w-[200px] h-full flex flex-col flex-shrink-0 border-r border-[#e8e8e8] select-none"
    style="background: rgba(255,255,255,0.8); backdrop-filter: blur(20px); -webkit-backdrop-filter: blur(20px);"
  >
    <!-- Logo area -->
    <div class="h-14 flex items-center px-5 border-b border-transparent cursor-pointer" @click="router.push('/')">
      <span class="text-[17px] font-bold text-[#1a1a1a] tracking-tight">EchoMusic</span>
    </div>
    ...
    <nav class="flex-1 overflow-y-auto px-2 py-2">
      ...
      <div class="mt-5">
        <div class="flex items-center justify-between px-3 mb-2">
          ...
          <button class="text-[#999] hover:text-[#333] transition-colors min-h-[28px] min-w-[28px] flex items-center justify-center" @click="user.createPlaylist('新歌单')">
    ...
    <div class="px-2 py-2 border-t border-[#f0f0f0] space-y-[1px]">
      <div
        class="px-3 py-2 rounded-lg cursor-pointer text-[13px] transition-colors"
        :class="isActive('/settings') ? 'bg-[#fef0f0] text-[#ec4141] font-semibold' : 'text-[#666] hover:bg-[#f5f5f5]'"
      ...
      <div
        class="px-3 py-2 rounded-lg cursor-pointer text-[13px] transition-colors"
        :class="isActive('/login') ? 'bg-[#fef0f0] text-[#ec4141] font-semibold' : 'text-[#666] hover:bg-[#f5f5f5]'"
